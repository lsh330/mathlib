...
def hypercosec(
    ...
    sinh_value = complex(
        hypersin(x, tol=tol_value, max_terms=max_terms_value, number_system=number_system_value)
    )
    if abs(sinh_value) <= tol_value:
        raise ZeroDivisionError("sinh(x) is zero, csch(x) is undefined.")

    normalized = _normalize(1.0 / sinh_value, tol_value)
    if number_system_value == "real":
