...
class Differentiation:
    """
    Numerical differentiation toolkit built from derivative definitions.

    Every method in this class is implemented from limit-based difference
    quotients (forward/backward/central), not symbolic derivative identities.
    """
    ...
    def _validate_positive_real(self, name: str, value: object) -> float:
    ...
    def _validate_positive_int(self, name: str, value: object) -> int:
    ...
    def _validate_scalar_input(self, name: str, value: object, number_system: str) -> Number:
    ...
    def _evaluate_operator_function(
        self,
    ...
    def _get_vector_component_function(
        self,
        vector_field: VectorFunction,
        component_index: int,
        point: Sequence[Number],
        number_system: str,
        tol: float,
    ...
    def single_variable(
        ...
        for _ in range(max_iter_value):
            current = self.difference_quotient(
                ...
                h=h_current,
                method=method,
                number_system=number_system_value,
                tol=tol_value,
            )
    ...
    def nth_derivative(
        ...
        # For second derivative, directly use the limit definition:
        ...
        if order_value == 2:
            ...
            for _ in range(max_iter_value):
                f_xh = self._evaluate_scalar_function(
                    function, [x_value + h_current], number_system_value, tol_value
                )
                f_x = self._evaluate_scalar_function(function, [x_value], number_system_value, tol_value)
                f_xmh = self._evaluate_scalar_function(
                    function, [x_value - h_current], number_system_value, tol_value
                )

                current = self._normalize(
                    (f_xh - 2.0 * f_x + f_xmh) / (h_current * h_current),
                    tol_value,
                    number_system_value,
                )
            ...
            raise RuntimeError("Second derivative did not converge within max_iter.")

        def lower_order(t: Number) -> Number:
    ...
    def parametric_derivative(
        self,
        coordinate_functions: Sequence[ScalarFunction],
        parameter: Number,
        ...
        def coordinate_value(index: int, t: Number) -> Number:
            return self._evaluate_scalar_function(
                coordinate_functions[index], [t], number_system_value, tol_value
            )

        def first_order_ratio(t: Number) -> Number:
            d_dependent_dt = self.single_variable(
                lambda tau: coordinate_value(dependent_index, tau),
                t,
                h=h,
                method=method,
                number_system=number_system_value,
            ...
            d_independent_dt = self.single_variable(
                lambda tau: coordinate_value(independent_index, tau),
                t,
                h=h,
                method=method,
                number_system=number_system_value,
            ...
            if self._abs(d_independent_dt) <= tol_value:
                raise ZeroDivisionError("Parametric derivative undefined because dx/dt is approximately zero.")
            return self._normalize(d_dependent_dt / d_independent_dt, tol_value, number_system_value)

        if order_value == 1:
            return first_order_ratio(parameter_value)

        current_function: ScalarFunction = first_order_ratio
        for _ in range(2, order_value + 1):
            previous_function = current_function

            def next_order_ratio(t: Number, _prev: ScalarFunction = previous_function) -> Number:
                d_prev_dt = self.single_variable(
                    _prev,
                    t,
                    h=h,
                    method=method,
                ...
                if self._abs(d_independent_dt) <= tol_value:
                    raise ZeroDivisionError("Higher-order parametric derivative undefined because dx/dt is approximately zero.")
                return self._normalize(d_prev_dt / d_independent_dt, tol_value, number_system_value)
    ...
    def partial_derivative(
        ...
        def single_axis(t: Number) -> Number:
            ...
            tol_value_local = self._resolve_tol(tol)
            return self._evaluate_scalar_function(function, shifted, number_system_value, tol_value_local)

        return self.single_variable(
            single_axis,
            point_value[variable_index],
            h=h,
            method=method,
            number_system=number_system_value,
            tol=tol,
    ...
    def mixed_partial(
        ...
        function: ScalarFunction,
        point: Vector,
        order_multi_index: Sequence[int],
        h: Optional[float] = None,
        method: str = "central",
        number_system: Optional[str] = None,
        tol: Optional[float] = None,
    ...
    def higher_order_directional_derivative(
        self,
        function: ScalarFunction,
        point: Vector,
        direction: Vector,
    ...
    def hessian_vector_product(
        ...
        h_current = self._resolve_h(h)
        previous: Optional[List[Number]] = None

        for _ in range(max_iter_value):
            gradient_at_point = self.gradient(
                function,
                point_value,
                h=h_current,
                method=method,
            ...
            for i in range(len(point_value)):
                shifted_point.append(point_value[i] + h_current * vector_value[i])

            gradient_shifted = self.gradient(
                function,
                shifted_point,
                h=h_current,
                method=method,
            ...
            for i in range(len(point_value)):
                current.append(
                    self._normalize(
                        (gradient_shifted[i] - gradient_at_point[i]) / h_current,
                        tol_value,
                        number_system_value,
                    )
                )
            ...
            if previous is not None:
                ...
                for i in range(len(current)):
                    delta = self._abs(current[i] - previous[i])
                    if delta > max_diff:
                        max_diff = delta
                    magnitude = self._abs(current[i])
                    if magnitude > max_current:
                        max_current = magnitude
                if max_diff <= tol_value * max_current:
                    return current

            previous = current
            h_current *= 0.5

        raise RuntimeError("Hessian-vector product did not converge within max_iter.")
    ...
    def total_derivative(
        self,
        outer_function: ScalarFunction,
        inner_functions: Sequence[ScalarFunction],
        point: Vector,
        h: Optional[float] = None,
        method: str = "central",
        number_system: Optional[str] = None,
        ...
        """
        Total derivative of composition F(x) = outer_function(inner_functions(x)).

        inner_functions: R^n -> R^m (as m scalar callables)
        outer_function: R^m -> R
        """
        number_system_value = self._resolve_number_system(number_system)
        point_value = self._validate_point(point, number_system_value)
        tol_value = self._resolve_tol(tol)
        ...
        for inner_function in inner_functions:
            inner_point.append(
                self._evaluate_scalar_function(inner_function, point_value, number_system_value, tol_value)
            )

        outer_gradient = self.gradient(
            outer_function,
            inner_point,
            h=h,
            method=method,
    ...
    def implicit_derivative(
        ...
        number_system_value = self._resolve_number_system(number_system)
        point = [
            self._validate_scalar_input("x", x, number_system_value),
            self._validate_scalar_input("y", y, number_system_value),
        ]
        tol_value = self._resolve_tol(tol)

        fx = self.partial_derivative(
            relation,
            point,
            variable_index=0,
            h=h,
    ...
    def implicit_partial(
        ...
        f_independent = self.partial_derivative(
            relation,
            point_value,
            variable_index=independent_index,
            h=h,
            method=method,
            number_system=number_system_value,
            tol=tol_value,
        ...
        f_dependent = self.partial_derivative(
            relation,
            point_value,
            variable_index=dependent_index,
            h=h,
            method=method,
            number_system=number_system_value,
            tol=tol_value,
        ...
        if self._abs(f_dependent) <= tol_value:
            raise ZeroDivisionError("Implicit partial derivative undefined because denominator is approximately zero.")

        return self._normalize(-f_independent / f_dependent, tol_value, number_system_value)

    # ---------------------------------------------------------------------
    # Complex and generalized derivatives
    # ---------------------------------------------------------------------
    def wirtinger_derivatives(
        ...
        def plane_function(x_real: Number, y_real: Number) -> Number:
            return self._evaluate_scalar_function(
                function,
                [complex(x_real, y_real)],
                "complex",
                tol_value,
            )

        partial_x = self.partial_derivative(
            plane_function,
            [x_value, y_value],
            variable_index=0,
            h=h,
        ...
        partial_y = self.partial_derivative(
            plane_function,
            [x_value, y_value],
            variable_index=1,
            h=h,
        ...
        df_dz_conjugate = self._normalize(0.5 * (partial_x + 1j * partial_y), tol_value, number_system_value)

        return {
            "point": complex(x_value, y_value),
            "df_dz": df_dz,
            "df_dz_conjugate": df_dz_conjugate,
        }
    ...
    def subgradient(
        ...
        tol: Optional[float] = None,
        max_iter: Optional[int] = None,
    ) -> Union[List[Number], Tuple[Number, Number]]:
        info = self.generalized_derivative(
            function,
            x,
            h=h,
            tol=tol,
    ...
    def gateaux_derivative(
        ...
        for _ in range(max_iter_value):
            ...
            current = self._divide_operator_output(
                self._subtract_operator_outputs(shifted_value, base_value),
                h_current,
            )
            current = self._normalize_operator_output(current, tol_value, number_system_value)

            if previous is not None:
                if self._operator_output_difference(current, previous) <= tol_value * max(
                    1.0, self._operator_output_norm(current)
                ):
                    return current
        ...
        if isinstance(output_at_point, list):
            ...
            for index in range(output_dimension):
                ...
                def component_function(*args: Number, _index: int = index) -> Number:
                    ...
                    values = self._evaluate_operator_function(
                        function,
                        point_candidate,
                        number_system_value,
                        tol_value,
    ...
    def divergence(
        ...
        for i in range(dimension):
            component = self._get_vector_component_function(
                vector_field, i, point_value, number_system_value, tol_value
            )
            total = total + self.partial_derivative(
                component,
                point_value,
                variable_index=i,
                h=h,
    ...
    def curl(
        ...
        d_fz_dy = self.partial_derivative(
            fz,
            point_value,
            variable_index=1,
            h=h,
        ...
        d_fy_dz = self.partial_derivative(
            fy,
            point_value,
            variable_index=2,
            h=h,
        ...
        d_fx_dz = self.partial_derivative(
            fx,
            point_value,
            variable_index=2,
            h=h,
        ...
        d_fz_dx = self.partial_derivative(
            fz,
            point_value,
            variable_index=0,
            h=h,
        ...
        d_fy_dx = self.partial_derivative(
            fy,
            point_value,
            variable_index=0,
            h=h,
        ...
        d_fx_dy = self.partial_derivative(
            fx,
            point_value,
            variable_index=1,
            h=h,
        ...
        return [
            self._normalize(d_fz_dy - d_fy_dz, tol_value, number_system_value),
    ...
    def total_differential(
        ...
        """
        Return first-order total differential coefficients at a point.
        """
        number_system_value = self._resolve_number_system(number_system)
        point_value = self._validate_point(point, number_system_value)
        coefficients = self.gradient(
            function,
            point_value,
            h=h,
            method=method,
